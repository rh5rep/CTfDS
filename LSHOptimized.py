...
POLITICAL_STOP_WORDS = [
    ...
    'abcnews', 'cnn', 'fox', 'foxnews', 'msnbc', 'nbc', 'nbcnews', 'cbs', 'cbsnews', 'news']
...
# POLITICAL_STOP_WORDS = [
#     'trump', 'biden', 'donald', 'joe', 'potus', '2020', 'trumps', 'realdonaldtrump', 'harris',
#     'save', 'yep', 'yes', 'nope', 'no', 'fa', 'oh', 'nytimes', 'wow', 'tells', 'omg', 'wait', 'look',
#     'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']
# CUSTOM_STOP_WORDS = list(ENGLISH_STOP_WORDS) + POLITICAL_STOP_WORDS
# MAX_FEATURES = 3000


# def find_topic_clusters_parallel(docs, threshold=0.2, num_perm=120, q=4, 
#                                min_cluster_size=50, n_terms=6, batch_size=1000, 
#                                n_clusters=6, max_workers=8):
#     # Split documents into batches for parallel processing
#     doc_items = list(docs.items())
#     n_batches = math.ceil(len(doc_items) / batch_size)
#     batches = [dict(doc_items[i*batch_size:(i+1)*batch_size]) 
...
#             # Merge clusters with existing ones
#             for cluster_id, info in batch_clusters.items():
#                 if cluster_id not in all_clusters:
#                     all_clusters[cluster_id] = info
#                 else:
#                     all_clusters[cluster_id]['documents'].extend(info['documents'])
#                     all_clusters[cluster_id]['size'] += info['size']
...
#     TERMS = 6
#     docs = {i: tweet for i, tweet in enumerate(df['tweet'])}
#     docs = dict(list(docs.items())[:10000])
    
#     print("Processing hashtags...")
#     # Use parallel processing for documents
#     docs = dict(zip(docs.keys(), map(process_tweet, docs.values())))
    
#     # Find topic clusters using parallel processing
#     clusters = find_topic_clusters_parallel(
#         docs,
#         threshold=0.2,
#         num_perm=120,
#         q=4,
